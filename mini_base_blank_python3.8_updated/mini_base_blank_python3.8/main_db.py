                        ...
                        while True:
                            try:
                                ...
                                if 0 <= record_idx < len(records):
                                    ...
                                    for idx, (field_name, field_type, _) in enumerate(field_list):
                                        type_str = ["STRING", "VARSTRING", "INTEGER", "BOOLEAN"][field_type]
                                        current_value = records[record_idx][idx]
                                        print(f"{idx}. {field_name.decode('utf-8').strip()} ({type_str}): {current_value}")
                                    ...
                                    if 0 <= field_idx < len(field_list):
                                        field_name = field_list[field_idx][0].decode('utf-8').strip()
                                        field_type = field_list[field_idx][1]
                                        old_value = str(records[record_idx][field_idx])
                                        
                                        # 7. 获取并验证新值
                                        type_str = ["STRING", "VARSTRING", "INTEGER", "BOOLEAN"][field_type]
                                        new_value = input(f'Enter new value for {field_name} ({type_str}): ')
